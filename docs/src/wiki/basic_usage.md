...
```bash
qsim run-model --task-config examples/noise_simulation_tests/required_tasks/task1_single_qubit.yaml
```
...
- `task`：目标、输入电路、输出目录
- `solver`：引擎、时间步长、参考系和数值运行参数
- `device`：设备结构、物理参数、噪声参数
- `pulse`：通道、载频、波形和操作映射
- `analyser`：所有从 `trajectory` 出发的派生分析
...
5. 复制 `templates/analysers/default.yaml`
6. 先只改相对路径、输出目录和少量物理参数
...
input:
  ...
  analyser_config: ../../../templates/analysers/default.yaml
output:
  out_dir: ../runs/demo_trajectory
...
```bash
qsim run-model --task-config path/to/task.yaml
```

## 路径解析规则

`task.input.solver_config`、`device_config`、`pulse_config`、`analyser_config` 和 `qasm_path` 都会相对当前 `task.yaml` 所在目录解析。
...
qsim run-model ^
  ...
  --analyser-config templates/analysers/default.yaml
```
...
model.run()
print(model.out_dir)
print("solver_0" in model.results.trajectories)
print("analyser_0" in model.results.analyses["solver_0"])
```
...
1. 先跑 `trajectory`
2. 先用 `qutip`
3. 先确认 `trajectory.h5` 和 `pulse_ir.json` 正常生成
