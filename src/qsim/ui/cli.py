"""Command-line interface for task/solver/hardware-driven workflow execution."""
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(description="qsim workflow runner (task/solver/hardware)")
    sub = parser.add_subparsers(dest="cmd", required=True)

    run_task_cmd = sub.add_parser("run-task", help="run workflow from task/solver/hardware configs")
    ...
    run_task_cmd.add_argument("--hardware-config", default="", help="optional hardware config override (json/yaml)")
    return parser
...
def main() -> None:
    ...
    Example:
        ```bash
        qsim run-task --task-config tasks/demo.yaml --solver-config solvers/qutip.yaml --hardware-config hw/default.yaml
        ```
    """
    parser = build_parser()
