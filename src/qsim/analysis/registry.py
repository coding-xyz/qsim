...
class AnalysisRegistry:
    ...
    def register(self, name: str, callable_obj: callable, schema_in: str, schema_out: str) -> str:
        ...
        Args:
            name: Pass identifier used by ``AnalysisRunner``.
            callable_obj: Callable implementing the pass contract.
            schema_in: Declared input schema marker (documentation/traceability).
            schema_out: Declared output schema marker.

        Returns:
            A short revision string derived from registration timestamp.
        """
...
class AnalysisRunner:
    ...
    def run(self, trace: Trace, model_spec: ModelSpec, pipeline: str = "default") -> dict:
        ...
        Supports ``pipeline="custom:<name>"`` aliasing, and injects
        ``analysis_rev``/``analysis_name`` into the returned payload.
        """
