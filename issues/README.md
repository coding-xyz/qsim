# Issues Index

状态说明：
- `In Progress`：已立项，仍在执行。
- `Done`：按 issue 验收完成并已归档到 `issues/done/`。
- `Failed`：历史失败项，仅留档参考。
- `Template`：新建 issue 使用模板。

完成规则：
1. 代码、测试、`docstring`、`docs/` 都完成后，issue 才能标记完成。
2. `docs/site/` 视为构建产物，优先维护 `docs/src/`，并通过构建同步。
3. 标记完成后，issue 文件移动到 `issues/done/`。

## In Progress

- [ISSUE_DYN_P0_MODEL_SPEC_ENGINE_NEUTRAL_IR.md](./ISSUE_DYN_P0_MODEL_SPEC_ENGINE_NEUTRAL_IR.md)
- [ISSUE_NOISE_P0_SYSTEMATIC_NOISE_IR_AND_CROSSTALK_SCHEMA.md](./ISSUE_NOISE_P0_SYSTEMATIC_NOISE_IR_AND_CROSSTALK_SCHEMA.md)
- [ISSUE_UI_P0_MODELSPEC_BOUNDARY_HARDENING_AND_RESULT_ANALYSIS_RESTRUCTURE.md](./ISSUE_UI_P0_MODELSPEC_BOUNDARY_HARDENING_AND_RESULT_ANALYSIS_RESTRUCTURE.md)

## Done

- [ISSUE_DYN_P1_QUTIP_ENGINE_STRUCTURAL_REFACTOR.md](./done/ISSUE_DYN_P1_QUTIP_ENGINE_STRUCTURAL_REFACTOR.md)
- [ISSUE_DYN_P1_QUTIP_ENGINE_SME_REFACTOR_AND_PHOTON_COUNTING.md](./done/ISSUE_DYN_P1_QUTIP_ENGINE_SME_REFACTOR_AND_PHOTON_COUNTING.md)
- [ISSUE_DYN_P0_JULIA_ENGINE_PULSE_SCHEDULE_PARITY_AND_RENAME.md](./done/ISSUE_DYN_P0_JULIA_ENGINE_PULSE_SCHEDULE_PARITY_AND_RENAME.md)
- [ISSUE_DYN_P0_TRI_ENGINE_DYNAMICS_FULLSTACK.md](./done/ISSUE_DYN_P0_TRI_ENGINE_DYNAMICS_FULLSTACK.md)
- [ISSUE_DYN_P1_TRI_ENGINE_TRACE_OBSERVABLE_ALIGNMENT.md](./done/ISSUE_DYN_P1_TRI_ENGINE_TRACE_OBSERVABLE_ALIGNMENT.md)
- [ISSUE_PULSE_P1_BREAKABLE_PULSE_POLICY_FROM_LOWERING_AND_CATALOG.md](./done/ISSUE_PULSE_P1_BREAKABLE_PULSE_POLICY_FROM_LOWERING_AND_CATALOG.md)
- [ISSUE_PULSE_P1_CHANNEL_LAYOUT_SPLIT_OR_MERGED.md](./done/ISSUE_PULSE_P1_CHANNEL_LAYOUT_SPLIT_OR_MERGED.md)
- [ISSUE_PULSE_P1_GATE_TO_PULSE_MAPPING_CATALOG.md](./done/ISSUE_PULSE_P1_GATE_TO_PULSE_MAPPING_CATALOG.md)
- [ISSUE_PULSE_P1_LOWERING_SCHEDULING_POLICY_SERIAL_PARALLEL_HYBRID.md](./done/ISSUE_PULSE_P1_LOWERING_SCHEDULING_POLICY_SERIAL_PARALLEL_HYBRID.md)
- [ISSUE_QEC_P0_DECODER_MWPM_BP.md](./done/ISSUE_QEC_P0_DECODER_MWPM_BP.md)
- [ISSUE_QEC_P0_SCHEMA_AND_INTERFACES.md](./done/ISSUE_QEC_P0_SCHEMA_AND_INTERFACES.md)
- [ISSUE_QEC_P0_STIM_CIRQ_PRIOR.md](./done/ISSUE_QEC_P0_STIM_CIRQ_PRIOR.md)
- [ISSUE_QEC_P1_DECODER_EVAL_AND_SWEEP.md](./done/ISSUE_QEC_P1_DECODER_EVAL_AND_SWEEP.md)
- [ISSUE_QEC_P1_REAL_JULIA_ENGINE_BRIDGE.md](./done/ISSUE_QEC_P1_REAL_JULIA_ENGINE_BRIDGE.md)
- [ISSUE_QEC_P1_SENSITIVITY_AND_ERROR_BUDGET.md](./done/ISSUE_QEC_P1_SENSITIVITY_AND_ERROR_BUDGET.md)
- [ISSUE_QEC_P2_PAULI_PLUS_ERROR_BUDGET.md](./done/ISSUE_QEC_P2_PAULI_PLUS_ERROR_BUDGET.md)
- [ISSUE_QEC_P2_SCALE_AND_RELIABILITY.md](./done/ISSUE_QEC_P2_SCALE_AND_RELIABILITY.md)
- [ISSUE_UI_P0_MODEL_OBJECT_API_AND_SOLVER_ANALYSER_SPLIT.md](./done/ISSUE_UI_P0_MODEL_OBJECT_API_AND_SOLVER_ANALYSER_SPLIT.md)
- [ISSUE_UI_P0_WORKFLOW_FINAL_SHAPE_TASK_ONLY.md](./done/ISSUE_UI_P0_WORKFLOW_FINAL_SHAPE_TASK_ONLY.md)
- [ISSUE_UI_P1_WORKFLOW_REFACTOR_TASK_DRIVEN_PIPELINE.md](./done/ISSUE_UI_P1_WORKFLOW_REFACTOR_TASK_DRIVEN_PIPELINE.md)
- [ISSUE_UI_P1_WORKFLOW_TARGET_DEPENDENCY_TEMPLATE_PLAN.md](./done/ISSUE_UI_P1_WORKFLOW_TARGET_DEPENDENCY_TEMPLATE_PLAN.md)
- [ISSUE_UI_P1_WORKFLOW_ARTIFACT_WRITER_STRUCTURED_SERVICE.md](./done/ISSUE_UI_P1_WORKFLOW_ARTIFACT_WRITER_STRUCTURED_SERVICE.md)
- [ISSUE_UI_P0_WORKFLOW_TASK_SOLVER_HARDWARE_SPLIT_AND_TARGET_SCHEMA.md](./done/ISSUE_UI_P0_WORKFLOW_TASK_SOLVER_HARDWARE_SPLIT_AND_TARGET_SCHEMA.md)

## Template

- [ISSUE_TEMPLATE.md](./ISSUE_TEMPLATE.md)

## Failed

- [ISSUE_DYN_P0_QUANTUM_CLASSICAL_READOUT_CHAIN_AND_TASK2_SPAM.md](./failed/ISSUE_DYN_P0_QUANTUM_CLASSICAL_READOUT_CHAIN_AND_TASK2_SPAM.md)
- [ISSUE_DYN_P1_TASK1_SINGLE_QUBIT_VISUAL_AND_NATIVE_REFERENCE.md](./failed/ISSUE_DYN_P1_TASK1_SINGLE_QUBIT_VISUAL_AND_NATIVE_REFERENCE.md)
- [ISSUE_QEC_P0_ROADMAP.md](./failed/ISSUE_QEC_P0_ROADMAP.md)
- [ISSUE_UI_P1_NOTEBOOK_CONFIG_AND_RESULT_HELPERS.md](./failed/ISSUE_UI_P1_NOTEBOOK_CONFIG_AND_RESULT_HELPERS.md)
